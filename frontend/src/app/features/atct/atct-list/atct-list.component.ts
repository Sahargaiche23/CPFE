                      ...
                      <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                          <a [routerLink]="['/atct/detail', dossier.id]"
                             class="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Voir">
                          ...
                          <a [routerLink]="['/atct/edit', dossier.id]"
                             class="p-1.5 text-yellow-600 hover:bg-yellow-50 rounded" title="Modifier">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                            </svg>
                          </a>
                          <button (click)="deleteDossier(dossier)"
                                  class="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Supprimer">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export class AtctListComponent implements OnInit {
  ...
  deleteDossier(dossier: DossierATCT): void {
    if (!dossier.id) return;
    const nom = dossier.nomCompletFr || `${dossier.prenomFr} ${dossier.nomFr}`;
    if (!confirm(`Voulez-vous vraiment supprimer le dossier de ${nom} ?\n\nCette action est irréversible.`)) {
      return;
    }
    this.atctService.delete(dossier.id).subscribe({
      next: () => {
        this.allDossiers = this.allDossiers.filter(d => d.id !== dossier.id);
