...
export class DebitListComponent implements OnInit {
      ...
      next: (result) => {
        ...
        this.debits = (result.debits as any[]).map(d => {
          ...
          return {
            ...
            dateLimite: d.dateLimite ? new Date(d.dateLimite).toLocaleDateString('fr-FR') : '-',
            rappelEnvoye: d.rappelEnvoye || false,
  ...
  autoGenerateDebits() {
    ...
    if (!confirm(`Générer automatiquement les débits pour T${currentQuarter}/${currentYear} pour tous les coopérants affiliés ?`)) {
      return;
    }
    
    this.autoGenerating = true;
    this.http.post<any>('/api/debits/auto-generate', {
