...
export class PdfService {
  ...
  /**
   * Génère un PDF multi-pages (1 page par trimestre) avec le même template RTL arabe
   */
  async generateMultiPageDebitPdfBase64(debits: {
    number: string;
    employerName: string;
    trimestre: string;
    generatedDate: string;
    ...
    for (let i = 0; i < debits.length; i++) {
      if (i > 0) doc.addPage();
      const canvas = await this.renderDebitPageToCanvas(debits[i]);
      const imgData = canvas.toDataURL('image/jpeg', 0.8);
      doc.addImage(imgData, 'JPEG', 0, 0, 210, 297);
    }
  ...
  private renderDebitPageToCanvas(debit: {
    number: string; employerName: string; trimestre: string; generatedDate: string; amount: number;
    numAffiliation?: string; matricule?: string; salaire?: number; adresse?: string; annee?: number;
    cotisations?: { code: string; nomAr: string; taux: number; base: number; montant: number }[];
    ...
    return new Promise((resolve, reject) => {
      ...
      const iframeDoc = iframe.contentDocument || iframe.contentWindow?.document;
      if (!iframeDoc) { reject('Cannot access iframe document'); return; }
  ...
  /**
   * Génère le formulaire ATCT officiel bilingue (FR/AR) - même design que attestation_affiliation
   */
  generateAtctFormulaire(data: {
    nomComplet: string;
    nomCompletAr?: string;
    ...
    iframeDoc.write(`
      ...
      <body>
        ...
        <div class="section">
          ...
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">الإسم و اللقب</span><br>
              <span class="field-label-fr">Nom et Prénom</span>
            </div>
            <div class="field-value">${data.nomComplet || '..............................'}</div>
          </div>
          ${data.nomCompletAr ? `
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-fr">)en arabe(</span>
            </div>
            <div class="field-value">${data.nomCompletAr}</div>
          </div>` : ''}
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">رقم الضمان الاجتماعي</span><br>
              <span class="field-label-fr">N° Sécurité Sociale</span>
            </div>
            <div class="field-value">${data.numSecuSociale || '..............................'}</div>
          </div>
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">العنوان الإلكتروني</span><br>
              <span class="field-label-fr">Email</span>
            </div>
        ...
        <div class="section">
          ...
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">العنوان بتونس</span><br>
              <span class="field-label-fr">Adresse en Tunisie</span>
            </div>
            <div class="field-value">${data.adresseTunisie || ''} ${data.codePostal || ''} ${data.villeTunisie || ''}</div>
          </div>
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">العنوان بالخارج</span><br>
              <span class="field-label-fr">Adresse à l'Étranger</span>
            </div>
            <div class="field-value">${data.adresseEtranger || ''} ${data.villeEtranger || ''} - ${data.paysEtranger || ''}</div>
          </div>
        </div>
        ...
        <div class="section">
          <div class="section-title">
            ...
            <span class="section-title-fr">DÉTACHEMENT</span>
            <div class="clear"></div>
          </div>
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">المؤسسة الأصلية بتونس</span><br>
              <span class="field-label-fr">Établissement d'origine</span>
            </div>
            <div class="field-value">${data.etablissementOrigine || '..............................'}</div>
          </div>
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">الهيئة المستقبلة بالخارج</span><br>
              <span class="field-label-fr">Organisme d'accueil</span>
            </div>
            <div class="field-value">${data.organismeEtranger || '..............................'}</div>
          </div>
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">بداية الإلحاق</span><br>
              <span class="field-label-fr">Début détachement</span>
            </div>
            <div class="field-value">${data.dateDebutDetachement || '....../....../......'}</div>
          </div>
          <div class="field-row">
            <div class="field-label">
              <span class="field-label-ar">نهاية الإلحاق</span><br>
              <span class="field-label-fr">Fin détachement</span>
            </div>
            <div class="field-value">${data.dateFinDetachement || '....../....../......'}</div>
          </div>
        </div>
        ...
        <div class="section">
          ...
          <div class="checkbox-section">
            <div class="checkbox-row">
              <span class="checkbox">${checkAnticipe}</span> مسبقا / Par anticipation
              &nbsp;&nbsp;&nbsp;&nbsp;
              <span class="checkbox">${checkTrimestriel}</span> كل ثلاثة أشهر / Trimestriel
            </div>
            <div class="checkbox-row">
              <span class="checkbox">${checkAnnuel}</span> سنويا / Annuel
              &nbsp;&nbsp;&nbsp;&nbsp;
              <span class="checkbox">${checkMensuel}</span> شهريا / Mensuel
            </div>
          </div>
        </div>
        ...
        <div class="option-box">
          <div class="checkbox-row">
            <span class="checkbox">${checkAssurance}</span>
            <strong>التأمين على المرض / Assurance Maladie</strong><br>
            <span style="font-size: 10px; margin-right: 20px;">أرغب في الإنتفاع بنظام الحيطة الإجتماعية (تغطية صحية + رأس المال عند الوفاة)</span><br>
            <span style="font-size: 10px; direction: ltr;">Je souhaite bénéficier du régime de prévoyance sociale (couverture maladie + capital décès)</span>
          </div>
          <div class="checkbox-row" style="margin-top: 10px;">
            <span class="checkbox">${checkNoAssurance}</span>
            لا أرغب في الإنتفاع / Je ne souhaite pas bénéficier de ce régime
          </div>
        </div>
        
        <!-- Signature -->
        <div class="signature-section">
          <p>حررت في .................. في ${dateStr} &nbsp;&nbsp;&nbsp; Fait à .................. le ${dateStr}</p>
          <br><br>
    ...
    setTimeout(() => {
      ...
      html2canvas(iframeDoc.body, { 
        ...
        const imgData = canvas.toDataURL('image/png');
        doc.addImage(imgData, 'PNG', 0, 0, 210, 297);
        doc.save('Formulaire_ATCT_' + (data.numSecuSociale || 'CNSS') + '.pdf');
        
        document.body.removeChild(iframe);
      }).catch((err: any) => {
        console.error('Erreur génération PDF ATCT:', err);
        document.body.removeChild(iframe);
      });
    }, 2000);
  }
